$(document).ready(function(){
	$("#art_filter").click(function(){
		...
		window.document.location.href = site_url + 'adm_illneses/illneses/' + nPerPage + '/' + nOrder + '/' + sDirect + '/' + nArtId + '~' + sTitle + '~' + sArtStatus + '/0';
		return false;
	});
	$("#art_filter_reset").click(function(){
		...
		var sDirect = $('#art_direct').val();
		window.document.location.href = site_url + 'adm_illneses/illneses/' + nPerPage + '/' + nOrder + '/' + sDirect + '/~~~/0';
		return false;
	});
	...
	$('.save').click(function(){
		...
		$.post(site_url + 'adm_illneses/illnese_order_save', {id: id, order: order}, function(data){
			if( data == 'ok' ){
	...
	$(function() {
		...
		$( "#art_title" ).autocomplete({
			...
			source: function( request, response ) {
				...
				lastXhr = $.getJSON( site_url + "adm_illneses/illnese_autocomplete_title", request, function( data, status, xhr ) {
					cache[ term ] = data;
					if ( xhr === lastXhr ) {
						response( data );
					}
