$(document).ready(function(){
	$("#art_filter").click(function(){
		...
		window.document.location.href = site_url + 'adm_trans/essential_infos/' + nPerPage + '/' + nOrder + '/' + sDirect + '/' + nArtId + '~' + sTitle + '/0';
		return false;
	});
	$("#art_filter_reset").click(function(){
		...
		var sDirect = $('#art_direct').val();
		window.document.location.href = site_url + 'adm_trans/essential_infos/' + nPerPage + '/' + nOrder + '/' + sDirect + '/~~/0';
		return false;
	});
	...
	$(function() {
		...
		$( "#art_title" ).autocomplete({
			...
			source: function( request, response ) {
				...
				lastXhr = $.getJSON( site_url + "adm_trans/essential_info_autocomplete_title", request, function( data, status, xhr ) {
					cache[ term ] = data;
					if ( xhr === lastXhr ) {
						response( data );
					}
