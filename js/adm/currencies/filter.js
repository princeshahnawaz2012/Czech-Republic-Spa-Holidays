$(document).ready(function(){
	$("#art_filter").click(function(){
		...
		window.document.location.href = site_url + 'adm_currencies/currencies/' + nPerPage + '/' + nOrder + '/' + sDirect + '/' + nArtId + '~' + sTitle + '~' + sArtStatus + '/0';
		return false;
	});
	$("#art_filter_reset").click(function(){
		...
		var sDirect = $('#art_direct').val();
		window.document.location.href = site_url + 'adm_currencies/currencies/' + nPerPage + '/' + nOrder + '/' + sDirect + '/~~/0';
		return false;
	});
	...
	$('.save').click(function(){
		...
		var id = input.attr('id').substr(0, 3);
		var order = parseInt(input.val());
		if(isNaN(order)){
			order = 0;
		}
		$.post(site_url + 'adm_currencies/currency_order_save', {id: id, order: order}, function(data){
			if( data == 'ok' ){
