...
{form_open_multipart()}
	...
	<fieldset>
		<div class="grid_4">
			...
			<input type="text" name="title" value="{set_value('title', $aImageData['title'])}" autocomplete="off" />
		</div>
		<div class="grid_4">
			<label>{vlang('Status')} <small>{vlang('Required')}</small></label>
			<select name="com_active">
				<option value="{$PROGRAMME_IMAGE_ACTIVE}" {set_select('com_active', $PROGRAMME_IMAGE_ACTIVE, $PROGRAMME_IMAGE_ACTIVE == $aImageData['com_active'])}>{vlang('Active')}</option>
				<option value="{$PROGRAMME_IMAGE_INACTIVE}" {set_select('com_active', $PROGRAMME_IMAGE_INACTIVE, $PROGRAMME_IMAGE_INACTIVE == $aImageData['com_active'])}>{vlang('Inactive')}</option>
			</select>
		</div>
		<div class="grid_2">
			<label>{vlang('Order')} <small>{vlang('Integer')}</small></label>
			<input type="text" name="com_order" value="{set_value('com_order', $aImageData['com_order'])}" autocomplete="off" />
		</div>
		<div class="clearfix"></div>
		<div class="grid_2">
			<label>{vlang('Picture')}</label>
			<input type="hidden" name="com_image_ext" id="new_com_picture_ext" value="{$aImageData['com_image_ext']}" />
