	...
	<fieldset>
		...
		<div class="grid_4">
			...
			<select name="com_active">
				<option value="{$ILLNESE_ACTIVE}" {set_select('com_active', $ILLNESE_ACTIVE)}>{vlang('Active')}</option>
				<option value="{$ILLNESE_INACTIVE}" {set_select('com_active', $ILLNESE_INACTIVE)}>{vlang('Inactive')}</option>
			</select>
		</div>
		<div class="grid_4">
