...
{form_open_multipart()}
	...
	<fieldset>
		...
		<div class="grid_5">
			...
			<select name="com_region_id" id="new_region_title">
				...
				{foreach $aRegions as $aRegion}
					<option value={$aRegion.region_id} {set_select('com_region_id', $aRegion.region_id)}>{$aRegion.title}</option>
				{/foreach}
			</select>
		</div>
		<div class="grid_2">
		...
		<div class="grid_16">
			...
			<textarea name="desc" id="new_desc" class="tinymce">{set_value('desc')}</textarea>
		</div>
		<div class="clearfix"></div>
		<div class="grid_6">
			<label>{vlang('Map')}</label>
			<input type="hidden" name="com_map_ext" id="new_com_map_ext" value="" />
			<a class="button" id="button_map_upload" href="#">{vlang('Image')}</a>
			
		</div>
		<div class="grid_3">
			<label>{vlang('Emblem')}</label>
			<input name="emblem_label" id="new_emblem_label" type="text" value="" autocomplete="off" />
			<input type="hidden" name="com_emblem_ext" id="new_com_emblem_ext" value="" />
		</div>
		<div class="grid_2">
			<label>&nbsp;</label>
		...
		<div class="grid_3">
			...
			<input name="flag_label" id="new_flag_label" type="text" value="{set_value('flag_label')}" autocomplete="off" />
			<input type="hidden" name="com_flag_ext" id="new_com_flag_ext" value="" />
		</div>
		<div class="grid_2">
			<label>&nbsp;</label>
