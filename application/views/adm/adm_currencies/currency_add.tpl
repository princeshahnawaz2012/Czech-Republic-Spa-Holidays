...
{form_open_multipart()}
	...
	<fieldset>
		<div class="grid_2">
			...
			<input type="text" name="com_currency_id" value="{set_value('com_currency_id')}" autocomplete="off" />
		</div>
		<div class="grid_6">
		...
		<div class="grid_4">
			...
			<select name="com_active">
				<option value="{$CURRENCY_INACTIVE}" {set_select('com_active', $CURRENCY_INACTIVE)}>{vlang('Inactive')}</option>
				<option value="{$CURRENCY_ACTIVE}" {set_select('com_active', $CURRENCY_ACTIVE)}>{vlang('Active')}</option>
			</select>
		</div>
