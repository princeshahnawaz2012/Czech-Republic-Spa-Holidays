	...
	<fieldset>
		...
		<div class="grid_4">
			...
			<select name="com_active">
				<option value="{$ESSENTIAL_INFO_ACTIVE}" {set_select('com_active', $ESSENTIAL_INFO_ACTIVE)}>{vlang('Active')}</option>
				<option value="{$ESSENTIAL_INFO_INACTIVE}" {set_select('com_active', $ESSENTIAL_INFO_INACTIVE)}>{vlang('Inactive')}</option>
			</select>
		</div>
		<div class="grid_4">
