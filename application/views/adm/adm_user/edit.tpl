...
{form_open()}
	...
	<fieldset>
		<div class="grid_4">
			...
			<input type="text" name="login" value="{set_value('login', $aData.user.login)}" autocomplete="off" />
		</div>
		<div class="grid_4">
			<label>Full name <small>{vlang('Required')}</small></label>
			<input type="text" name="fio" value="{set_value('fio', $aData.user.fio)}" autocomplete="off" />
		</div>	
		<div class="grid_4">
			<label>Email <small>{vlang('Required')}</small></label>
			<input type="text" name="email" value="{set_value('email', $aData.user.email)}" autocomplete="off" />
		</div>			
		<div class="grid_4">
			<label>Role <small>{vlang('Required')}</small></label>
			{html_options id="role" name=role options=$aData.role selected=set_value('role',$aData.user.role)}
		</div>
		<div class="grid_8">
