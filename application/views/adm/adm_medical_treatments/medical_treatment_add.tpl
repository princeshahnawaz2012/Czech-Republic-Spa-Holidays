	...
	<fieldset>
		...
		<div class="grid_4">
			...
			<select name="com_active">
				<option value="{$MEDICAL_TREATMENT_ACTIVE}" {set_select('com_active', $MEDICAL_TREATMENT_ACTIVE)}>{vlang('Active')}</option>
				<option value="{$MEDICAL_TREATMENT_INACTIVE}" {set_select('com_active', $MEDICAL_TREATMENT_INACTIVE)}>{vlang('Inactive')}</option>
			</select>
		</div>
		<div class="grid_4">
