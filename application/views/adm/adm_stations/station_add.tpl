	...
	<fieldset>
		...
		<div class="grid_4">
			...
			<select name="com_active">
				<option value="{$STATION_ACTIVE}" {set_select('com_active', $STATION_ACTIVE)}>{vlang('Active')}</option>
				<option value="{$STATION_INACTIVE}" {set_select('com_active', $STATION_INACTIVE)}>{vlang('Inactive')}</option>
			</select>
		</div>
		<div class="grid_4">
