...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ID" value="{$aFilters["essential_infos.com_essential_info_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_essential_infos.title"]}" />
	</p>
</div>
<div class="grid_4">
	<p>
...
{foreach $aEssential_infos as $essential_info}
	<tr class="{cycle values="first,alt"}">
		...
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $essential_info[$aLang|cat:"_title"]}
					<a class="edit" href="{$site_url}adm_trans/essential_info_edit/{$essential_info.com_essential_info_id}/{$aLang}" title="Edit this translate"></a>
				{else}
					<a class="new" href="{$site_url}adm_trans/essential_info_edit/{$essential_info.com_essential_info_id}/{$aLang}" title="Input this translate"></a>
				{/if}
			</td>
		{/foreach}
	</tr>	
