...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ID" value="{$aFilters["facilities.com_facility_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_facilities.title"]}" />
	</p>
</div>
<div class="grid_4">
	<p>
...
{foreach $aFacilities as $facility}
	<tr class="{cycle values="first,alt"}">
		...
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $facility[$aLang|cat:"_title"]}
					<a class="edit" href="{$site_url}adm_trans/facility_edit/{$facility.com_facility_id}/{$aLang}" title="Edit this translate"></a>
				{else}
					<a class="new" href="{$site_url}adm_trans/facility_edit/{$facility.com_facility_id}/{$aLang}" title="Input this translate"></a>
				{/if}
			</td>
		{/foreach}
	</tr>	
