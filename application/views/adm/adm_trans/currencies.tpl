...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ISO" value="{$aFilters["com_currency_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_currencies.title"]}" />
	</p>
</div>
<div class="grid_4">
	<p>
...
{foreach $aCurrencies as $currency}
	<tr class="{cycle values="first,alt"}">
		<td>{$currency.com_currency_id}</td>
		<td>{$currency[$sMainLang|cat:"_title"]}</td>
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $currency[$aLang|cat:"_title"]}
					<a class="edit" href="{$site_url}{$sEditUrl}{$currency.com_currency_id}/{$aLang}" title="Edit this translate"></a>
				{else}
					<a class="new" href="{$site_url}{$sEditUrl}{$currency.com_currency_id}/{$aLang}" title="Input this translate"></a>
				{/if}
			</td>
		{/foreach}
	</tr>	
