...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ID" value="{$aFilters["com_country_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_countries.title"]}" />
	</p>
</div>
<div class="grid_2">
	<p>
		<label for="art_iso">ISO</label>
		<input type="text" id="art_iso" placeholder="ISO" value="{$aFilters["com_iso"]}" />
	</p>
</div>
<div class="grid_4">
	<p>
...
{foreach $aCountries as $country}
	<tr class="{cycle values="first,alt"}">
		...
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $country[$aLang|cat:"_title"]}
					<a class="edit" href="{$site_url}adm_trans/country_edit/{$country.com_country_id}/{$aLang}" title="Edit this translate"></a>
				{else}
					<a class="new" href="{$site_url}adm_trans/country_edit/{$country.com_country_id}/{$aLang}" title="Input this translate"></a>
				{/if}
			</td>
		{/foreach}
	</tr>	
