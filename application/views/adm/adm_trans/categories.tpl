...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ID" value="{$aFilters["com_category_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_categories.title"]}" />
	</p>
</div>
<div class="grid_4">
	<p>
...
{foreach $aCategories as $category}
	<tr class="{cycle values="first,alt"}">
		<td>{$category.com_category_id}</td>
		<td>{$category[$sMainLang|cat:"_title"]}</td>
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $category[$aLang|cat:"_title"]}
					<a class="edit" href="{$site_url}{$sEditUrl}{$category.com_category_id}/{$aLang}" title="Edit this translate"></a>
				{else}
					<a class="new" href="{$site_url}{$sEditUrl}{$category.com_category_id}/{$aLang}" title="Input this translate"></a>
				{/if}
			</td>
		{/foreach}
	</tr>	
