...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ID" value="{$aFilters["stations.com_station_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_stations.title"]}" />
	</p>
</div>
<div class="grid_4">
	<p>
...
{foreach $aStations as $station}
	<tr class="{cycle values="first,alt"}">
		...
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $station[$aLang|cat:"_title"]}
					<a class="edit" href="{$site_url}adm_trans/station_edit/{$station.com_station_id}/{$aLang}" title="Edit this translate"></a>
				{else}
					<a class="new" href="{$site_url}adm_trans/station_edit/{$station.com_station_id}/{$aLang}" title="Input this translate"></a>
				{/if}
			</td>
		{/foreach}
	</tr>	
