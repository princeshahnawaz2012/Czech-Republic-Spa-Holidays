...
<div class="grid_2">
	<p>
		...
		<input type="text" id="art_id" placeholder="ID" value="{$aFilters["com_article_id"]}" />
	</p>
</div>
<div class="grid_6">
	<p>
		<label for="art_title">Title</label>
		<input type="text" id="art_title" placeholder="Title" value="{$aFilters[$sMainLang|cat:"_articles.title"]}" />
...
{foreach $aArticles as $article}
	<tr class="{cycle values="first,alt"}">
		<td>{$article.com_article_id}</td>
		<td>{$article[$sMainLang|cat:"_title"]}</td>
		{foreach $aLangPermissions as $aLang}
			<td>
				{if $article[$aLang|cat:"_editing_end"] < $nTime}
					{if $article[$aLang|cat:"_title"]}
						<a class="edit" href="{$site_url}adm_trans/article_edit/{$article.com_article_id}/{$aLang}" title="Edit this translate"></a>
					{else}
						<a class="new" href="{$site_url}adm_trans/article_edit/{$article.com_article_id}/{$aLang}" title="Input this translate"></a>
					{/if}
				{else}
					{if $article[$aLang|cat:"_title"]}
