...
/**
 * Description of medical_treatments_model
 * @author Sergii Lapin
 */
class Medical_treatments_model extends CI_Model
{

	private $table = 'medical_treatments';
	private $table_join = 'com_medical_treatment_id';
	private $table_fields = array('com_medical_treatment_id', 'com_active', 'com_order');

	private $subtable = '_medical_treatments';
	private $subtable_join = 'medical_treatment_id';
	private $subtable_fields = array('medical_treatment_id', 'title', 'short_desc');
	
	private $spas_medical_treatments_table = 'spas_medical_treatments';
	private $spas_medical_treatments_join = 'com_medical_treatment_id';
	...
	*
	* @param array $aMedical_treatmentData
	* @param int|array $aMedical_treatmentId
	* @return int
	*/
	function save($aMedical_treatmentData, $aMedical_treatmentId = NULL)
	{
		if ( $aMedical_treatmentId )
		{
			if ( is_array($aMedical_treatmentId) )
			{
				$this->db->where_in('com_medical_treatment_id', $aMedical_treatmentId);
			}
			else
			{
				$this->db->where('com_medical_treatment_id', $aMedical_treatmentId);
			}
			$this->db->update($this->table, $aMedical_treatmentData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($this->table, $aMedical_treatmentData);
	...
	* @return int
	*/
	function save_translate($sLang, $aMedical_treatmentData, $aMedical_treatmentId = NULL)
	{
		if ( $aMedical_treatmentId )
		{
			if ( is_array($aMedical_treatmentId) )
			{
				$this->db->where_in('medical_treatment_id', $aMedical_treatmentId);
			}
			else
			{
				$this->db->where('medical_treatment_id', $aMedical_treatmentId);
			}
			$this->db->update($sLang . $this->subtable, $aMedical_treatmentData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($sLang . $this->subtable, $aMedical_treatmentData);
			return $this->db->insert_id();
		}
	}
	...
	*/
	function delete($aMedical_treatmentId)
	{
		$aAffectedRows = array();
		if ( is_array($aMedical_treatmentId) )
		{
		...
		if ( is_array($aMedical_treatmentId) )
		{
			$this->db->where_in($this->spas_medical_treatments_join, $aMedical_treatmentId);
		}
		else
		{
			$this->db->where($this->spas_medical_treatments_join, $aMedical_treatmentId);
		}
		$this->db->delete($this->spas_medical_treatments_table);
	...
	* @return int
	*/
	function delete_translate($sLang, $aMedical_treatmentId)
	{
		if ( is_array($aMedical_treatmentId) )
		{
			$this->db->where_in('medical_treatment_id', $aMedical_treatmentId);
	...
	*/
	...
	/**
	* Получение процедуры или списка процедур. Основной информации, перевода на нужный язык и перевод на язык по умолчанию
	*
	* @param string $sLang
	* @param array $aFilters - фильры отбора ( array(поле => значение,...) )
	* @param boolean $bSingle - один или список (TRUE/FALSE)
...
/* End of file medical_treatments_model.php */
/* Location: ./application/models/medical_treatments_model.php */