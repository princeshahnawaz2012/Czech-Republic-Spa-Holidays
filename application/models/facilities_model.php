...
class Facilities_model extends CI_Model
{
	...
	private $table_join = 'com_facility_id';
	private $table_fields = array('com_facility_id', 'com_active', 'com_order');

	private $subtable = '_facilities';
	private $subtable_join = 'facility_id';
	private $subtable_fields = array('facility_id', 'title', 'short_desc');
	
	private $spas_facilities_table = 'spas_facilities';
	private $spas_facilities_join = 'com_facility_id';
	...
	*
	* @param array $aFacilityData
	* @param int|array $aFacilityId
	* @return int
	*/
	function save($aFacilityData, $aFacilityId = NULL)
	{
		if ( $aFacilityId )
		{
			if ( is_array($aFacilityId) )
			{
				$this->db->where_in('com_facility_id', $aFacilityId);
			}
			else
			{
				$this->db->where('com_facility_id', $aFacilityId);
			}
			$this->db->update($this->table, $aFacilityData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($this->table, $aFacilityData);
	...
	* @return int
	*/
	function save_translate($sLang, $aFacilityData, $aFacilityId = NULL)
	{
		if ( $aFacilityId )
		{
			if ( is_array($aFacilityId) )
			{
				$this->db->where_in('facility_id', $aFacilityId);
			}
			else
			{
				$this->db->where('facility_id', $aFacilityId);
			}
			$this->db->update($sLang . $this->subtable, $aFacilityData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($sLang . $this->subtable, $aFacilityData);
			return $this->db->insert_id();
		}
	}
	...
	*/
	function delete($aFacilityId)
	{
		...
		if ( is_array($aFacilityId) )
		{
			$this->db->where_in($this->spas_facilities_join, $aFacilityId);
		}
		else
		{
			$this->db->where($this->spas_facilities_join, $aFacilityId);
		}
		$this->db->delete($this->spas_facilities_table);
	...
	* @return int
	*/
	function delete_translate($sLang, $aFacilityId)
	{
		if ( is_array($aFacilityId) )
		{
			$this->db->where_in('facility_id', $aFacilityId);
	...
	*/
	...
	/**
	* Получение услуги на територии или списка услуг на територии. Основной информации, перевода на нужный язык и перевод на язык по умолчанию
	*
	* @param string $sLang
	* @param array $aFilters - фильры отбора ( array(поле => значение,...) )
	* @param boolean $bSingle - один или список (TRUE/FALSE)
...
/* Location: ./application/models/facilities_model.php */