...
class Regions_model extends CI_Model
{
	...
	private $table_join = 'com_region_id';
	private $table_fields = array('com_region_id', 'com_country_id', 'com_active', 'com_order');

	private $subtable = '_regions';
	private $subtable_join = 'region_id';
	private $subtable_fields = array('region_id', 'title');
	...
	*
	* @param array $aRegionData
	* @param int|array $aRegionId
	* @return int
	*/
	function save($aRegionData, $aRegionId = NULL)
	{
		if ( $aRegionId )
		{
			if ( is_array($aRegionId) )
			{
				$this->db->where_in('com_region_id', $aRegionId);
			}
			else
			{
				$this->db->where('com_region_id', $aRegionId);
			}
			$this->db->update($this->table, $aRegionData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($this->table, $aRegionData);
	...
	* @return int
	*/
	function save_translate($sLang, $aRegionData, $aRegionId = NULL)
	{
		if ( $aRegionId )
		{
			if ( is_array($aRegionId) )
			{
				$this->db->where_in('region_id', $aRegionId);
			}
			else
			{
				$this->db->where('region_id', $aRegionId);
			}
			$this->db->update($sLang . $this->subtable, $aRegionData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($sLang . $this->subtable, $aRegionData);
			return $this->db->insert_id();
		}
	}
	...
	* @return int
	*/
	function delete_translate($sLang, $aRegionId)
	{
		if ( is_array($aRegionId) )
		{
			$this->db->where_in('region_id', $aRegionId);
	...
	*/
	...
	/**
	* Получение региона или списка регионов. Основной информации, перевода на нужный язык и перевод на язык по умолчанию
	*
	* @param string $sLang
	* @param array $aFilters - фильры отбора ( array(поле => значение,...) )
	* @param boolean $bSingle - один или список (TRUE/FALSE)
	...
	*/
	...
	public function get_adm_list($aAdditionalLang = array(), $aFilters = false, $bSingle = false, $aLimit = NULL, $aOrder=NULL, $bFilterLike = FALSE)
	{
		...
		$this->db->join($sMainLang . '_countries', $this->table . '.com_country_id = ' . $sMainLang . '_countries.country_id', 'left');
		...
		if ( is_array($aAdditionalLang) )
		{
			foreach ( $aAdditionalLang as $sLang )
			{
				if ( $sLang != $sMainLang )
				{
					...
					$this->db->join($sLang . $this->subtable, $this->table . '.' . $this->table_join . ' = ' . $sLang . $this->subtable . '.' . $this->subtable_join, 'left');
					$this->db->select($sLang . '_countries.title as `' . $sLang . '_country_title`');
					$this->db->join($sLang . '_countries', $this->table . '.com_country_id = ' . $sLang . '_countries.country_id', 'left');
				}
			}
		}
	...
	public function get_count_adm_list($aFilters = false, $bFilterLike = FALSE)
	{
		...
		$this->db->from($this->table);
		$this->db->join($sMainLang . $this->subtable, $this->table . '.' . $this->table_join . ' = ' . $sMainLang . $this->subtable . '.' . $this->subtable_join, 'left');
		$this->db->select('countries.title as `' . $sMainLang . '_country_title`');
		$this->db->join($sMainLang . '_countries', $this->table . '.com_country_id = ' . $sMainLang . '_countries.country_id', 'left');
		if ( $aFilters )
		{
			if ( $bFilterLike )
