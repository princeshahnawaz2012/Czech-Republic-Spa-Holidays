...
class Stations_model extends CI_Model
{
	...
	private $table_join = 'com_station_id';
	private $table_fields = array('com_station_id', 'com_active', 'com_order');

	private $subtable = '_stations';
	private $subtable_join = 'station_id';
	private $subtable_fields = array('station_id', 'title');
	...
	*
	* @param array $aStationData
	* @param int|array $aStationId
	* @return int
	*/
	function save($aStationData, $aStationId = NULL)
	{
		if ( $aStationId )
		{
			if ( is_array($aStationId) )
			{
				$this->db->where_in('com_station_id', $aStationId);
			}
			else
			{
				$this->db->where('com_station_id', $aStationId);
			}
			$this->db->update($this->table, $aStationData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($this->table, $aStationData);
	...
	* @return int
	*/
	function save_translate($sLang, $aStationData, $aStationId = NULL)
	{
		if ( $aStationId )
		{
			if ( is_array($aStationId) )
			{
				$this->db->where_in('station_id', $aStationId);
			}
			else
			{
				$this->db->where('station_id', $aStationId);
			}
			$this->db->update($sLang . $this->subtable, $aStationData);
			return $this->db->affected_rows();
		}
		else
		{
			$this->db->insert($sLang . $this->subtable, $aStationData);
			return $this->db->insert_id();
		}
	}
	...
	* @return int
	*/
	function delete_translate($sLang, $aStationId)
	{
		if ( is_array($aStationId) )
		{
			$this->db->where_in('station_id', $aStationId);
